...
class OurMNIST_dataset(torch.utils.data.Dataset):
...
def get_dataset(args):
    ...
    if args.case == 'OurMNIST'  and args.num_users == 5:
        dirname = os.path.dirname(__file__)
        data_dir = os.path.join(dirname,'../data/OurMNIST/')
        ...
        for i in range(1, num_users+1):
            ...
            train_dataset[i] = OurMNIST_dataset(random.sample(dictionary['trainset_'+traindivision_index[0]+'_'+str(i)],
    ...
    elif args.case == 'OurMNIST' and args.num_users == 10:
        dirname = os.path.dirname(__file__)
        data_dir = os.path.join(dirname,'../data/OurMNIST/')
        # data_dir = '../data/OurMNIST/'
        ...
        for i in range(1, int(num_users/2)+1):
            ...
            train_dataset[2*i-1] = OurMNIST_dataset(random.sample(dictionary['trainset_'+traindivision_index[0]+'_'+str(i)],
                                        length)) 
...
def calculate_gradients(new_weights, old_weights):
    ...
    """
    gradients = copy.deepcopy(new_weights)
    for key in gradients.keys():
        gradients[key] -= old_weights[key]
...
def calculate_accumulate_gradients(sum_gradients, new_weights, old_weights):
    """
    :param sum_gradients: gradients accumulated from all rounds
    :param new_weights: list of weights generated from the users
    :param old_weights: old weights of a model, probably before training
    :Returns the list of gradients.
    """
    gradients = copy.deepcopy(sum_gradients)
    for key in gradients.keys():
        gradients[key] += (new_weights[key] - old_weights[key])
    return gradients
